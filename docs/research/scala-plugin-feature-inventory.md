...
| Surface | Classification and inheritance decision | Evidence and required proof |
|---|---|---|
| Application discovery, gutter and run configuration | Discovery is a symbol/model consumer; process launch is execution-only. Keep runners and patchers unchanged. | `ScalaApplicationConfigurationProducer` is at `scala-plugin-common.xml:547`; main gutter at line 453; Scala 3 synthetic main finder at line 519. Test discovery, configuration, execution, rerun and source navigation. |
| ScalaTest, MUnit, Specs2 and uTest | Framework discovery is type/resolve/annotation sensitive; execution is unchanged. This is not proven by general type tests. | Four `testFramework` registrations are in `scalaCommunity.testing-support.xml:23` and `scalaCommunity.testing-support.munit.xml:9`; producers are at lines 31 and 13. Each framework needs gutter, configuration, class/test name mapping, execution, failure navigation and rerun-failed coverage. |
| Debugger, evaluator and smart step | Alternate code-fragment model plus type/resolve consumers. Runtime debugger infrastructure stays; evaluator inputs and source mapping need focused parity. | `ScalaCodeFragmentFactory` is registered in `scalaCommunity.debugger.xml:12`; smart-step, position manager, renderer and evaluator-related extensions are cataloged there. A dedicated execution-parity issue is required. |
| Worksheets, scratch files, Ammonite, Scala console and REPL | Deferred semantic integration. Existing execution remains owned by the bundled plugin; editor presentation may consume backend snapshots later. | Worksheet registrations include run markers and goto handlers in `scalaCommunity.worksheet.xml:34-68`; REPL is included by `plugin.xml:87`. Owned by #67. |
