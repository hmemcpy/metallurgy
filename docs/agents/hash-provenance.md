...
| Test lane suites and invocations | Each `test-lanes/*.invocations.txt` is its lane's reviewed source of truth. Its tracked `test-lanes/*.txt` companion is the generated suite view. The packet adding, removing, or moving an invocation owns the source row and reviews the generated view. Copied-test manifests remain separate authority. | `python3 scripts/test_lane_mapping.py write` generates only suite views from contiguous invocation groups. Review readable row changes before accepting them. | `python3 scripts/test_lane_mapping.py check`, then `scripts/test-test-lane-runner.sh`, then run `scripts/run-test-lane.sh <lane-manifest> --run-id <new-id>` with isolated `METALLURGY_TEST_EVIDENCE_DIR` | Name the lane and show missing, extra, reordered, duplicate, malformed, wildcard, empty, non-contiguous, or copied generated-suite rows with expected and actual indexes. Missing discovery, zero-invocation reports, and filtered zero-test runs must fail. Hash-only output is not enough. |
