...
| Persistence schema and catalog fingerprint | `src/main/scala/com/hmemcpy/metallurgy/psiproducer/Scala3PsiProductionCatalog.scala`, `Scala3PsiCatalogModel.scala`, `Scala3PsiProductionSupport.scala`, the ten `Scala3Psi*Productions.scala` inputs in that directory, and `src/main/scala/com/hmemcpy/metallurgy/psiproducer/Scala3DotcFileElementType.scala`; persistence changes belong to the catalog and stub packet | None today. The catalog computes the value in process, but no command emits separate readable persistence and plan structures. | `sbt "testOnly com.hmemcpy.metallurgy.psiproducer.Scala3PsiProductionCatalogTest"` | Show changed external IDs, serializers, index obligations, persisted fields, catalog rows, and plan roles before the digest. The current combined value is a known gap and will be split in a later cleanup packet. |
...
| Exact tool and host pins | `build.sbt`, `project/build.properties`, nested build files, and host configuration named by the owning subsystem | None today. Pins are repeated and do not yet have one baseline generator. | Reload each affected build under the exact JBR and compare resolved coordinates; run `sbt check` for the root build | Name the mismatched coordinate, build, JBR, or resolved artifact. A later cleanup packet will provide one baseline manifest and verifier. |
