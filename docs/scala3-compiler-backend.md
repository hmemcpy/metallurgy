...
This document records the consumer surface and the bundled type-resolution background that the producer replaces.
The target architecture — producing source-compatible PSI from the Scala 3 compiler, so the bundled plugin's tests pass
unchanged — is in **epic #73** (Phases 0–8, issues #74–#82). Prior ADR and research documents have been removed.
...
### Target architecture — PSI producer

For an active Scala 3 module, Metallurgy produces source-compatible IntelliJ Scala PSI from the compiler's typed
output, and the bundled Scala plugin's compiler-valid tests pass unchanged against that PSI. Where the bundled PSI
parses a region correctly, the bundled tree is used; where the bundled PSI mis-parses a region the compiler typed
(`PsiErrorElement`s, fragmented recovery — e.g. constructs the installed plugin version cannot yet parse), Metallurgy
substitutes a compiler-built, source-preserving PSI subtree, so every consumer reads ordinary, correct PSI.

The per-element `CompilerType` slot overlay described later in this document is a **transitional** mechanism for healthy
regions; it is superseded by PSI-tree production. The complete target architecture, data model, hook points,
safeguards, testing approach, migration path, and the 38-task implementation plan are in **epic #73** (Phases 0–8,
issues #74–#82). This document records the consumer surface and the bundled type-resolution background that the
producer replaces.

Production rules: the compiler is the source of truth for any source it accepts; Metallurgy never requires a newer
bundled plugin than the user has; a PSI subtree is produced only from a compiler-clean snapshot and never covers a range
...
refactoring framework, and the `ScType` compatibility shell — and the project-model/build owner. Metallurgy produces the
typed PSI subtrees an active module observes; it does not replace every IntelliJ subsystem.
...
- Replacing the Scala **parser**, **stubs**, **indices**, editor mechanics, project import, debugger, or build runner.
  (Producing typed **PSI subtrees** from dotc is *in scope* — it is the foundational principle above; the parser entry
  point, stub index, and other platform mechanics remain bundled.) IntelliJ's native sbt importer and IntelliJ's BSP
  importer remain the project-model owners; Metallurgy does not speak sbt or BSP. Worksheet and interactive REPL
  semantic integration is deferred to a later phase, while their execution remains owned by the bundled Scala plugin.
...
| Application and test discovery | Scala main and `@main` producers inspect source PSI; ScalaTest, MUnit, Specs2, and uTest finders use `ScalaPsiManager` and inheritance against framework marker/base classes. Their registered configuration producers consume those same source identities. | **Indirectly.** Source-backed discovery inherits corrected resolve and remains bundled. Compiler-only executable targets require a stable light symbol acceptable to the producer. | Keep configuration classes, gutters, process launch, rerun, test result transport, and failure navigation bundled. Parity fixtures prove main/`@main`, all four suite finders/producers, and active/inactive gutter identity without starting a backend session. Actual launch/rerun remains a bounded execution test, not a new backend implementation. |
| Debugger fragments and smart step | `ScalaCodeFragmentFactory` creates synthetic contextual Scala PSI and selects lazy/eager bundled evaluator builders through `scala.debugger.expression.evaluator.lazy.resolve.enabled` (`intellij-scala/scala/debugger/.../ScalaCodeFragmentFactory.scala:28-91`). The fragment is module-associated but has no ordinary source-document snapshot generation. | **No current snapshot.** Its ordinary type/resolve remains bundled and context-correct. | Explicit safe fallback: never manufacture a source-document version or start a PC session for the fragment. Keep evaluator, transport, position mapping, renderers, and smart-step bundled. Both evaluator gate states and zero-work behavior are regression-tested. |
| Physical Scala worksheets | `WorksheetFile` extends `ScalaFileImpl`; plain and REPL modes are settings on the same physical, versioned file (`intellij-scala/scala/worksheet/src/org/jetbrains/plugins/scala/worksheet/WorksheetFile.scala:9-24`). | **Yes.** The normal semantic pass and cache contract apply without a worksheet adapter. | Keep compilation, execution, incremental result transport and reset/rerun in the bundled worksheet implementation. Current type/hover, edit retirement and inactive zero-work are regression-tested. The interactive console remains an explicit bundled fallback: `ScalaLanguageConsole` rebuilds nonphysical context PSI behind a `LightVirtualFile` (`.../scala/repl/src/org/jetbrains/plugins/scala/console/ScalaLanguageConsole.scala:181-266`), so there is no source document generation to commit against. |
...
inline, and override/implement must remain unavailable rather than mutating generated compiler state. A compiler-only
operation is enabled later only if it can be translated to a writable source declaration with an exact navigation
target; there is no text-edit fallback against light PSI.
...
| Compiler event topic | Build/compile-server lifecycle and diagnostics events. | Not a type-population mechanism. The compiler backend publishes from its own immutable whole-file snapshot; late CBH type reports cannot initiate per-offset queries or overwrite current backend state. | Subscribe only for a demonstrated lifecycle need, gate before work, and reject events whose module generation is not current. |
...
| `org.intellij.scala.referenceExtraResolver` | Fallback for stable code references, originally for Ammonite (`.../psi/impl/base/ScStableCodeReferenceExtraResolver.scala:11-38`). | **Insufficient as currently dispatched.** `resolveWithFileCheck` takes only the first registered extension rather than the first successful result (`.../ScStableCodeReferenceExtraResolver.scala:34-35`). Registering Metallurgy first can suppress Ammonite; registering it later can make Metallurgy unreachable. It also does not cover `ScReferenceExpression`. | Do not register while this first-extension behavior remains. Capability-probe the dispatcher and use a bridge-local stable-reference wrapper only when composition is unavailable. |
...
| Stable/type references | `ScStableCodeReferenceImpl` and `referenceExtraResolver` (`.../psi/impl/base/ScStableCodeReferenceImpl.scala:470-493`; `.../psi/impl/base/ScStableCodeReferenceExtraResolver.scala:11-38`) | Use the EP only if a behavior probe proves successful-result composition. The current implementation consults only its first extension, so the compatible adapter wraps stable-reference resolution, reads a current compiler symbol first, and otherwise delegates unchanged. | Preserve Ammonite and ordinary resolution exactly; the adapter is stable-reference-only and must not be generalized to expressions. |
...
| Source syntax the pinned bundled PSI cannot parse | The bundled Scala-plugin parser lags Scala 3 for some constructs dotc already accepts; the PSI mis-parses (error-recovery fragments) and `ScExpression.type()` cannot recover the typed result. **Named type arguments** (`f[A = Int]`, `import scala.language.experimental.namedTypeArguments`) are the worked example: fully supported upstream (`JetBrains/intellij-scala` `idea261.x`, merged `0e80ca7a99`/`8f1ff96741`) but absent from older runtimes (e.g. `2026.1.20`, rev `c2cc2b2009`), which parse `[A = Int]` positionally and emit `']' expected`. dotc types it (`-Xprint:typer`: `pair[A = Int][String](...)`, `construct[Coll = List][Int](...)`). | dotc is the oracle; Metallurgy resolves the type from the compiler's typed tree and surfaces it **independently of the bundled PSI parser** (a version-agnostic sidestep of the unparseable region), never requiring a newer bundled plugin. The same path covers any construct the pinned PSI cannot yet represent. |
...
- Permanent support uses published Scalameta interfaces, capability discovery, and isolated IntelliJ-side bridges.
  Structural or reflective access is permitted only inside those bridges after supported interfaces and extension points
  are exhausted. No version pin, bytecode fingerprint, or version allowlist selects an implementation.
- The pass infrastructure is reusable plumbing: compiler-backend population is its own pass; diagnostics and inlays are
  consumers of the same immutable snapshot.

The bet is precise: **one exact-version Scala 3 PC pass populates a whole-file semantic snapshot; Metallurgy produces
source-compatible Scala PSI from it (epic #73), through supported EPs first and isolated wrappers or replacements where
