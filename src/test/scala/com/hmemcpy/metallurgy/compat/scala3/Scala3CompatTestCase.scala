...
abstract class Scala3CompatTestCase extends ScalaLightCodeInsightFixtureTestCase:
  ...
  override protected def checkTextHasNoErrors(text: String): Unit =
    myFixture.configureByText(ScalaFileType.INSTANCE, text.trim)
    val file    = getFile.asInstanceOf[ScalaFile]
    val uri     = file.getVirtualFile.getUrl
    val manager = PcSessionManager.get(getProject)
    manager.declareProjection(uri, topLevelExpressionInsertions(file))
    try
      awaitBackendPublished()
      val _ = myFixture.testHighlighting(false, false, false, getFile.getVirtualFile)
    finally manager.clearProjection(uri)
  ...
  override protected def checkHasErrorAroundCaret(text: String): Unit =
    ...
    try
      awaitBackendPublished()
      val caretIndex                                  = text.trim.indexOf(CARET)
  ...
  protected def assertExprType(source: String): Unit =
    ...
    try
      ...
      val role   = Scala3CompilerBackend
        .get(getProject)
        .stateForActiveModule(expr, module, CompilerBackendRole.ExpressionExact)
      assertTrue(
        s"the compiler did not type the selected expression (ExpressionExact=$role); " +
          s"refusing to fall back to bundled typing for:\n${file.getText}",
        role.isInstanceOf[CompilerBackendState.Current]
      )
      assertEquals(s"type of '${expr.getText}'", expected, actual)
    finally manager.clearProjection(fileUri)

  // dotc rejects a bare top-level expression, so each one becomes the initializer of a synthetic value in the
  ...
  /** Asserts an upstream-oracle conflict: the verbatim snippet must NOT type-check under the exact-version compiler, so
    * dotc (via the backend) must produce an ERROR. This is the conflict-proof for an adjudicated oracle defect — it
    * actively reproduces the disagreement and fails if dotc ever starts accepting the snippet (forcing re-triage). It
    * is never used to satisfy a passing assertion and never suppresses a real error.
    */
  protected def assertDotcOracleConflict(text: String): Unit =
    myFixture.configureByText(ScalaFileType.INSTANCE, text.trim)
    val file    = getFile.asInstanceOf[ScalaFile]
    val uri     = file.getVirtualFile.getUrl
    val manager = PcSessionManager.get(getProject)
    ...
    try
      ...
      assertTrue(
        s"expected an upstream-oracle conflict (dotc must reject this snippet) but no ERROR was produced:\n${file.getText}",
        errors.nonEmpty
      )
    finally manager.clearProjection(uri)
