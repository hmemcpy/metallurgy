...
private[metallurgy] object Scala3PsiProductionCoverageReport:
  ...
  private def render(pattern: CatalogValuePattern): String = pattern match
    ...
    case CatalogValuePattern.ClassifiedNameExcept(value, excluded)         => s"NameExcept[$value,$excluded]"
    case CatalogValuePattern.ClassifiedNameValue(value, text)              => s"NameValue[$value,$text]"
    case CatalogValuePattern.NonLowercaseNameExcept(excluded)              => s"NonLowercaseNameExcept[$excluded]"
