...
private[metallurgy] final class PreparedProductionCatalog private (
    ...
    val unavailableRealizations: Set[(String, String)]
)
...
private[metallurgy] object PreparedProductionCatalog:
  def prepare(
    ...
    Either.cond(errors.isEmpty, new PreparedProductionCatalog(catalog, compiler, surfaces, Set.empty), errors)
  ...
  def prepareRuntimeSubset(
    ...
    Either.cond(
      errors.isEmpty,
      new PreparedProductionCatalog(catalog, compiler, surfaces, unavailableRealizations),
      errors.distinct
    )
