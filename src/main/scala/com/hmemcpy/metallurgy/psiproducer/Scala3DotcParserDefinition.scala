...
/** Parser definition for the Metallurgy Scala 3 dialect. Lexing and parsing delegate to the bundled Scala 3 parser;
  ...
  * dialect-aware extension lookup applies. A dialect-owned file element type keeps the stub root distinct (the stub
  * builder asks the view provider for the exact language).
  */
