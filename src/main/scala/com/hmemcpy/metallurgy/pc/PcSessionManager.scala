...
final class PcSessionManager private[pc] (project: Project, fetcher: MtagsFetcher) extends Disposable:
  ...
  /** Compatibility-fixture only: declare compiler-only insertions for a document so a verbatim top-level fragment
    ...
    */
  private[metallurgy] def declareProjection(fileUri: String, insertions: Seq[PcProjectionInsertion]): Unit =
    if insertions.isEmpty then { val _ = projectionOverrides.remove(fileUri) }
    else { val _ = projectionOverrides.put(fileUri, insertions) }

  private[metallurgy] def clearProjection(fileUri: String): Unit =
    val _ = projectionOverrides.remove(fileUri)
  ...
  private def snapshotFor(file: VirtualFile): Option[PcSnapshot] =
    readAction: () =>
      Option(FileDocumentManager.getInstance.getDocument(file)).map: document =>
        ...
        Option(projectionOverrides.get(file.getUrl)) match
          case Some(insertions) =>
            PcSnapshot(file.getUrl, version, PcCompilationProjection.withInsertions(text, insertions))
          case None             =>
            PcSnapshot(file.getUrl, version, text)
