...
final class PcSession private (
  ...
  private def treeDto(
      snapshot: PcSnapshot,
      extract: (Scala3PcBridge, () => PcSnapshotCurrency) => Option[CompilerTreeDto]
  ...
  private def querySemanticdbOccurrences(
    ...
    if !capabilities.semanticdb.isAvailable then None
      ...
      val future = compiler.semanticdbTextDocument(PcSourceUri.normalize(snapshot.fileUri), snapshot.compilerText)
      try
