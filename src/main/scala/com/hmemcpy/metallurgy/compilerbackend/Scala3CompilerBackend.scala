...
final class Scala3CompilerBackend(project: Project):
  ...
  private val AnyToken = "\\bAny\\b".r
  private def introducesAny(renderedType: String, canonicalText: String): Boolean =
    // createTypeFromText degrades a failed sub-term resolution to Any (getOrAny). If the reparse introduced a
    // standalone `Any` that the dotc rendering did not contain, the display text was lossy for a nested term and
    // the slot must not be published. `\bAny\b` excludes AnyVal/AnyRef, and faithful types carry their `Any` in both.
    AnyToken.findFirstIn(canonicalText).isDefined && AnyToken.findFirstIn(renderedType).isEmpty
